...
def test_configure_account_infinity():
    ...
    with pytest.raises(ValidationError, match=r'Special numeric values \(nan '
...
def test_rejected_config():
    ...
    with pytest.raises(ValidationError, match='The rejection_code field '
    ...
    wrong_config_data['config_data'] = 1500 * 'Щ'
    wrong_config_data = s.dumps(wrong_config_data)
    with pytest.raises(ValidationError, match='The length of config_data '
                       'exceeds 2000 bytes'):
        s.loads(wrong_config_data)
...
def test_prepare_transfer():
    ...
    with pytest.raises(ValidationError, match='max_locked_amount must be equal '
                       'or greater than min_locked_amount'):
    ...
    with pytest.raises(ValidationError, match='Invalid coordinator_id '
                       'for direct transfer.'):
...
def test_finalize_transfer():
    ...
    with pytest.raises(ValidationError, match='The length of transfer_note '
                       'exceeds 500 bytes'):
...
def test_account_transfer():
    ...
    with pytest.raises(ValidationError, match='The acquired_amount field '
                       'is zero, which is not allowed'):
    ...
    with pytest.raises(ValidationError, match='The sender field contains '
                       'non-ASCII characters'):
